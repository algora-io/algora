defmodule Algora.Github.PollerSupervisor do
  ...
  def add_repo(owner, name) do
    GenServer.call(__MODULE__, {:add_repo, owner, name})
  end
  ...
  @impl true
  def handle_call({:add_repo, owner, name}, _from, %{supervisor_pid: supervisor_pid} = state) do
    case add_child(supervisor_pid, owner, name, self()) do
  ...
  defp add_child(supervisor_pid, owner, name, poller_supervisor) do
    child_spec = %{
      id: "#{owner}/#{name}",
      start:
        {Poller, :start_link,
         [[repo_owner: owner, repo_name: name, supervisor: poller_supervisor]]}
    }
